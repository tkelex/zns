...
inline uint8_t zns_ns_lbads(NvmeNamespace *ns)
{
    /* NvmeLBAF */
...
static int zns_init_zone_geometry(NvmeNamespace *ns, Error **errp)
{
    ...
    n->num_zones = ns->size / lbasz / n->zone_size;	/* zone的数量，ns->size是femu启动的时候给的 */
	femu_debug("Zone: num_zones = %u, lbasz = %u, ns_size=%ld\n", n->num_zones, lbasz, ns->size);
...
//		femu_debug("receive open cmd, zone=%u\n", zone_idx);
        status = zns_do_zone_op(ns, zone, proc_mask, zns_open_zone, req);
		status = NVME_SUCCESS;	//不管原本的ZNS怎么做，只需要收到open命令就行
        break;
    case NVME_ZONE_ACTION_CLOSE:
        if (all) {
            proc_mask = NVME_PROC_OPENED_ZONES;
        }
//		femu_debug("receive close cmd, zone=%u\n", zone_idx);
...
//		femu_debug("receive finish cmd, zone=%u\n", zone_idx);
...
//		femu_debug("receive reset cmd, zone=%u\n", zone_idx);
        *resets = 1;
...
//	NvmeRwCmd *rw2 = (NvmeRwCmd *)&req->cmd;
//	uint32_t nlb2 = (uint32_t)le16_to_cpu(rw2->nlb) + 1;
//	femu_debug("slba=%lu, nlb=%u, req nlb=%u, nlb2=%u\n", slba, nlb, req->nlb, nlb2);
    status = nvme_check_mdts(n, data_size);
    if (status) {
        goto err;
    }
...
//	zone_info.lbasz = 1 << zns_ns_lbads(ns);
//	zone_info.nlb = nlb;
//	zone_info.zone_idx = zns_zone_idx(ns, slba);
//    assert(zone_info.zone_idx < n->num_zones);
//	zone_info.wp = slba - zone->d.zslba;
//
//	lat = zns_ssd_write(n->zns_ssd, req, &zone_info);
//	req->reqlat = lat;
//	req->expire_time += lat;
//	femu_debug("Write:  zone_idx=%u, nlb=%u, lat=%lu ns\n", zone_info.zone_idx, nlb, lat);
